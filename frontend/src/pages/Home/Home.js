...
import {
  updateUser,
  updateUserLend,
} from "../../components/user/helper/helper";
...
function Home() {
  ...
  const onLend = (id) => {
    ...
    updateBookLend(token, id, user._id, {
      lend: { _id: user._id, name: user.name },
    })
      .then((res) => {
        console.log(res);
              ...
              <div className="card1">
                ...
                {isAuthenticated() && !book.lend ? (
                  <div className="card__lend">
                    ...
                    <a className="card__lendBy__icon">
                      <ShoppingCartIcon onClick={() => onLend(book._id)} />
                    </a>
                  </div>
                ) : (
                  book.lend && (
                    <div className="card__lend">
                      <p className="card__lendBy">
                        Lended by: {book.lend.name}
                      </p>
                    </div>
                  )
                )}
